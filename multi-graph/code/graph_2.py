...
MY_MODEL_ROOT = os.path.join(MODEL_ROOT, 'graph_2')
...
with graph_2.as_default():
  with tf.name_scope("graph_2"):
    x = tf.placeholder(tf.float32, name="inputs")
    y = tf.placeholder(tf.float32, name="targets")
    w = tf.Variable(tf.constant(5.0), dtype=tf.float32, name="w_1")
    b = tf.Variable(tf.constant(8.0), dtype=tf.float32, name="b_1")
